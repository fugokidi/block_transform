...
def main():
    ...
    defense = globals()[configs.defense](configs).cuda()
    # defense = None

    print("=> Loaders are set.")
    if configs.evaluate:
        validate(val_loader, model, criterion, configs, logger, defense)
        return
    ...
    for epoch in range(configs.TRAIN.start_epoch, configs.TRAIN.epochs):
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, lr_schedule, defense, configs.TRAIN.half)

        # evaluate on validation set
        prec1 = validate(val_loader, model, criterion, configs, logger, defense)
...
def train(train_loader, model, criterion, optimizer, epoch, lr_schedule, defense, half=False):
