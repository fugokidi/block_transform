...
def nattack(model, test_loader, adv_model):
    ...
    for i, (inputs, targets) in enumerate(iterator):
        ...
        with torch.no_grad():
            probs = nn.Softmax(dim=1)(model(normalize(input_var)))
        ...
        for runstep in range(200):
            ...
            temp = []
            for x in modify_try:
                temp.append(cv2.resize(x.transpose(1,2,0), dsize=(288,288), interpolation=cv2.INTER_LINEAR).transpose(2,0,1))
            ...
            if runstep % 10 == 0:
                ...
                with torch.no_grad():
                    outputsreal = model(normalize(input_var)).data.cpu().numpy()[0]
...
#                     imsave(folder+classes[targets[0]]+'_'+str("%06d" % batch_idx)+'.jpg',inputs.transpose(1,2,0))
            ...
            clipinput = (clipdist + (np.tanh(newimg) * boxmul + boxplus)).reshape(npop,3,288,288) #.reshape(npop,3,32,32)
            ...
            with torch.no_grad():
                outputs = adv_model(normalize(input_var)).data.cpu().numpy()
